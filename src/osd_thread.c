...
void init_OSDWindows(void)
{
	...
	for (window = 0; window < OSD_MAX_WINDOW; window++) {
		...
		case OSD_WINDOW_ZERO:
			...
			strcpy(serverConfig->osdwin[window].file,
			       OSD_DATABASE"/win_0.yuyv");
			break;
		case OSD_WINDOW_ONE:
			serverConfig->osdwin[window].enable = FALSE;
		...
		case OSD_WINDOW_FIVE:
			...
			serverConfig->osdwin[window].transparency = FALSE;
			strcpy(serverConfig->osdwin[window].osdtext,
			       OSD_WINDOW_FIVE_STR);
			break;
		case OSD_WINDOW_SIX:
			serverConfig->osdwin[window].enable = FALSE;
...
void *osdThread(void)
{
	...
	while (!KillOsdThread) {
		while (!g_osdflag)
			usleep(5);
		memcpy(g_osdbuff[i], g_framebuff[i],
		...
		for (window = 0; window < OSD_MAX_WINDOW; window++) {
			...
			if (serverConfig->osdwin[window].enable == TRUE) {
				...
				if (window < OSD_WINDOW_FIVE) {
					fp = fopen(serverConfig->osdwin[window].file,
						   "rb");
					if (fp == NULL)
						continue;
					if (fread(osd_data_ptr, 1, osd_size,
						  fp) != osd_size)
						return NULL;
					fclose(fp);
				} else
					get_osd_string(serverConfig->osdwin[window].osdtext,
						       osd_data_ptr);
				ptri = osd_data_ptr;
				ptr = g_osdbuff[i];
				...
				while (cnt < serverConfig->osdwin[window].height) {
					fill_osd_data(ptr, ptri,
						      serverConfig->osdwin[window].width*2,
