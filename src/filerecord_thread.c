...
void get_video_filename(char *name, int val)
{
	...
	if (val == 0) {
		snprintf(name, 100, "%s/Videos/svs/raw/VID_%d%d%d_%d%d%d.raw",
		...
		snprintf(name, 100, "%s/Videos/svs/h264/VID_%d%d%d_%d%d%d.h264",
			 path, current.year, current.mon, current.day,
			 current.hour, current.min, current.sec);
		strcat(cmd, "/h264");
		if (system(cmd) < 0)
		...
		if (serverConfig->video.recordenable == TRUE) {
			...
			while (serverConfig->video.recordenable == TRUE) {
				...
				if (serverConfig->enable_osd_thread == TRUE) {
					...
					lock(&buf_lock);
					apply_algo(g_framebuff[i],
						   serverConfig->algo_type);
					if (fwrite(g_framebuff[i],
						   serverConfig->capture.framesize,
						   1, fp) < 0) {
						perror("WRITE");
						continue;
