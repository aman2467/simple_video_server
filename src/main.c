 ...
 * @file    : main.c
 *
 * @description : This file contains main function which creates necessary
 *		threads.
 *
...
void *filerecordThread(void *args);
void *osdThread(void *args);
void *jpegsaveThread(void *args);
void *displayThread(void *args);
void *nwstreamThread(void *args);
...
void usage(char *exename)
{
	...
	printf(GREEN"\t\tstream     "NONE":"YELLOW" To enable NW stream\n");
	printf(GREEN"\t\t-d <device>"NONE":"YELLOW" Video Device\n");
	printf(GREEN"\t\t-w <width> "NONE":"YELLOW" Capture width\n");
	printf(GREEN"\t\t-h <height>"NONE":"YELLOW" Capture height"NONE"\n");
	pr_dbg(YELLOW"\n/*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*=*/"NONE"\n\n");
	exit(0);
}
...
void Init_Server(SERVER_CONFIG *serverConfig)
{
	...
	serverConfig->capture.framesize = serverConfig->capture.width *
		serverConfig->capture.height*BPP;
	/* network settings */
...
{
	int threadStatus = 0;
	pthread_t tCaptureThread, tOsdThread, tFilerecordThread;
	pthread_t tJpegsaveThread;
	...
	if (serverConfig->enable_display_thread) {
		if (!serverConfig->enable_osd_thread)
			serverConfig->disp.sdl_frame =
				calloc(1, serverConfig->capture.framesize);
	...
	for (i = 0; i < NUM_BUFFER; i++) {
		...
		if (serverConfig->enable_osd_thread) {
			g_osdbuff[i] =
				(char *)calloc(1,
					       serverConfig->capture.framesize);
			memset(g_osdbuff[i], 0,
			       serverConfig->capture.framesize);
		}
	}
	if (serverConfig->enable_stream_thread) {
		g_streambuff =
			(char *)calloc(1, serverConfig->capture.framesize);
	...
	if (serverConfig->enable_videosave_thread == TRUE) {
		...
		if (pthread_create(&tFilerecordThread, NULL,
				   filerecordThread, NULL)) {
	...
	if (serverConfig->enable_imagesave_thread == TRUE) {
		...
		if (pthread_create(&tJpegsaveThread, NULL,
				   jpegsaveThread, NULL)) {
	...
	if (serverConfig->enable_display_thread) {
		KillDisplayThread = 0;
		if (pthread_create(&tDisplayThread, NULL,
				   displayThread, NULL)) {
	...
	if (serverConfig->enable_stream_thread) {
		KillStreamThread = 0;
		if (pthread_create(&tStreamThread, NULL,
				   nwstreamThread, NULL)) {
	...
	while (1) {
		...
		if ((recvfrom(sock_fd, userdata, 30, 0,
		...
		case COMMAND_SET_VIDEO_TYPE:
			arg1 = *((int *)data);
			if (serverConfig->enable_videosave_thread ||
			    serverConfig->enable_stream_thread) {
		...
		case COMMAND_SET_OSD_ON_IMAGE:
			arg1 = *((int *)data);
			if (serverConfig->enable_osd_thread &&
			    serverConfig->enable_imagesave_thread) {
		...
		case COMMAND_SET_OSD_ON_VIDEO:
			arg1 = *((int *)data);
			if (serverConfig->enable_osd_thread &&
			    serverConfig->enable_videosave_thread) {
		...
		if ((sendto(sock_fd, status, 10, 0,
			    (const struct sockaddr *)&cli_addr, clen)) < 0) {
			perror("sendto");
		}
#endif
		usleep(10);
