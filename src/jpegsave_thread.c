 ...
 * @return  : void
 */
void jpeg(FILE *dest, unsigned char *rgb, unsigned int width,
 ...
 */
unsigned char *yuyv2rgb(unsigned char *yuyv,
			unsigned int width,
			unsigned int height)
{
	size_t i, j;
	...
	for (i = 0; i < height; i++) {
		for (j = 0; j < width; j += 2) {
			...
			rgb[index * 3 + 0] = minmax(0,
						    (y0 + 359*v) >> 8, 255);
			rgb[index * 3 + 1] = minmax(0,
						    (y0 + 88*v - 183*u) >> 8,
						    255);
			rgb[index * 3 + 2] = minmax(0,
						    (y0 + 454*u) >> 8, 255);
			rgb[index * 3 + 3] = minmax(0, (y1 + 359*v) >> 8,
						    255);
			rgb[index * 3 + 4] = minmax(0,
						    (y1 + 88*v - 183*u) >> 8,
						    255);
			rgb[index * 3 + 5] = minmax(0, (y1 + 454*u) >> 8,
						    255);
		}
	}
	return rgb;
...
void get_image_filename(char *name, int val)
{
	...
	if (val == 0) {
		snprintf(name, 100,
			 "%s/Pictures/svs/jpeg/IMG_%d%d%d_%d%d%d.jpg",
			 path, current.year, current.mon, current.day,
			 current.hour, current.min, current.sec);
		strcat(cmd, "/jpeg");
		if (system(cmd) < 0)
			printf("Fail to create directory\n");
	} else if (val == 1) {
		snprintf(name, 100, "%s/Pictures/svs/raw/IMG_%d%d%d_%d%d%d.raw",
			 path, current.year, current.mon, current.day,
			 current.hour, current.min, current.sec);
		strcat(cmd, "/raw");
		if (system(cmd) < 0)
...
/*
 * @func    : JPEG save Thread main function
 * @arg1    : void
 * @return  : void
 */
void *jpegsaveThread(void)
{
	...
	while (!KillJpegsaveThread) {
		...
		case 0:
			apply_algo((char *)serverConfig->jpeg.framebuff,
				   serverConfig->algo_type);
			rgb = yuyv2rgb(serverConfig->jpeg.framebuff,
				       serverConfig->capture.width,
				       serverConfig->capture.height);
			jpeg(fp, rgb, serverConfig->capture.width,
			     serverConfig->capture.height,
			     serverConfig->jpeg.quality);
			free(rgb);
			break;
		case 1:
			fwrite(serverConfig->jpeg.framebuff,
			       serverConfig->capture.framesize, 1, fp);
			break;
		}
