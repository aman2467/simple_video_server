...
/*
 * @usage : This function sends a command to video server
 * using UDP socket.
 *
 * @arg1  : command value
...
int sendCommand(int command, char *arg)
{
	...
	ser_addr.sin_port = htons(VIDEO_SERVER_PORT);
	ser_addr.sin_addr.s_addr = inet_addr(g_video_server_ip);
	...
	quit_flag = TRUE;
	alarm(WAIT_TIME);
	if ((sendto(sock_fd, wrbuff, 30, 0,
		    (const struct sockaddr *)&ser_addr, slen)) < 0) {
		perror("sendto");
		return FAILURE;
	}
	if ((recvfrom(sock_fd, status, 10, 0,
		      (struct sockaddr *)&ser_addr, &slen)) < 0) {
		perror("sendto");
		return FAILURE;
	}
